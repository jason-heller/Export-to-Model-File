...
struct sHeader
{
    uint32 magic number & version id ('MOD4')
    uint32 numObjects;
};
struct sObjects
{
    uint32 materialId; (UNUSED)
    uint32 numVertices;
    uint32 numTexCoords;
    uint32 numNormals;
    uint32 numFaceIndices;
};
...
class ExportMOD(bpy.types.Operator, ExportHelper, IOOBJOrientationHelper):
    ...
    def execute(self, context):
        ...
        headerSize     = 8 + 4      # + 4 is the magic number (MOD4)
        objectsSize    = 40
        ...
        offsetVertexBoneStride  = []
        offsetVertexBoneId      = []
        offsetVertexBoneWeight  = []
        ...
        for polygon in mesh.polygons:
            for j in range(0,3):
                ...
                if not exists:
                    ...
                    faceIndices.append(len(newVertexArray) - 1)
        ...
        vertexBoneIdOffset.append( weightOffset )
